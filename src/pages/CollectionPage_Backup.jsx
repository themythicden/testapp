import { useState, useEffect } from "react";import { useSearchParams } from "react-router-dom";import { supabase } from "../lib/supabaseClient";

import CardGrid from "../components/CardGrid";import InviteUser from "../components/InviteUser";import FiltersSection from "../components/FiltersSection";

import { getVisibleCards } from "../utils/cardSelectors.js";

export default function CollectionPage() {const [searchParams] = useSearchParams();const collectionId = searchParams.get("id");const collectionName = searchParams.get("name");

const [user, setUser] = useState(null);const [collection, setCollection] = useState(null);const [collectionUsers, setCollectionUsers] = useState([]);

const [cards, setCards] = useState([]);const [userCards, setUserCards] = useState({});const [allUserCards, setAllUserCards] = useState({});

const [setFilter, setSetFilter] = useState("master");const [statusFilter, setStatusFilter] = useState("all");const [typeFilter, setTypeFilter] = useState([]);const [supertypeFilter, setSupertypeFilter] = useState([]);const [legalOnly, setLegalOnly] = useState(false);const [showMineOnly, setShowMineOnly] = useState(false);const [searchQuery, setSearchQuery] = useState("");const [sortBy, setSortBy] = useState("number");

const [selectedOwnerEmails, setSelectedOwnerEmails] = useState([]);

const myRole = collectionUsers.find(u => u.email === user?.email)?.role;

useEffect(() => {async function loadUser() {const { data } = await supabase.auth.getUser();
...
useEffect(() => {async function loadCollectionUsers() {if (!collectionId) return;
...
useEffect(() => {async function loadCollection() {if (!collectionId || !user) return;
...
useEffect(() => {async function loadCards() {if (!collection) return;
...
useEffect(() => {async function loadUserCards() {if (!user || cards.length === 0) return;
...
useEffect(() => {async function loadAllUserCards() {if (!collectionUsers.length || cards.length === 0) {console.log("======== SKIPPING LOAD ALL USER CARDS ========");console.log("collectionUsers.length:", collectionUsers.length);console.log("cards.length:", cards.length);return;}
...
const visibleCards = collection? getVisibleCards({cards,userCards,allUserCards,collectionUsers,isCollab: collection?.is_collab,setFilter,statusFilter,collection,searchQuery,sortBy,typeFilter,supertypeFilter,selectedOwnerEmails,legalOnly}): [];

const handleAdd = async (cardId, variant) => {if (!user) return;
...
const handleRemove = async (cardId, variant) => {if (!user) return;
...
console.log("======== RENDER STATE ========");console.log("USER:", user?.email);console.log("COLLECTION:", collection);console.log("COLLECTION IS_COLLAB:", collection?.is_collab);console.log("COLLECTION USERS:", collectionUsers);console.log("MY ROLE:", myRole);console.log("CARDS COUNT:", cards.length);console.log("USER_CARDS:", userCards);console.log("ALL_USER_CARDS:", allUserCards);console.log("VISIBLE CARDS COUNT:", visibleCards.length);
